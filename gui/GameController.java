...
public class GameController {
    ...
    private StackPane createVisualCell(int index, Cell data) {
        ...
        if (data instanceof DoorCell) {
            ...
            if (door.getRole() == Role.LAUGHER) {
                imgName = "door_bg.png"; 
            } else {
                imgName = "door_bg2.png"; 
            }
    ...
    private void runTurnSequence() {
        ...
        diceAnim.setOnFinished(e -> {
            ...
            try {
                // A. Intercept the visual face value you just generated on screen
                int visuallyRolledFace = Integer.parseInt(diceLabel.getText());
                
                // B. If the monster is confused, its actual movement step direction or size changes 
                // but the base intermediate landing zone is always exactly where they step first.
                int intermediatePos = (startPos + visuallyRolledFace) % Constants.BOARD_SIZE;

                // C. Execute the backend state change
                game.playTurn(); 
                
                // D. Capture where they ACTUALLY ended up after cells/effects executed
                int finalPos = activeMonster.getPosition();

                // E. Deduce log details safely
                ...
                logAction(activeMonster.getName() + " rolled a " + visuallyRolledFace + " and landed on " + cellTypeName.toUpperCase() + "!");

                // F. Track energy updates
                ...
                // G. Run the 2-stage animation cleanly
    ...
    private void triggerOpponentAI() {
        ...
        thinking.setOnFinished(e -> {
            ...
            try {
                // Generate a simulated dice roll for visual tracking matching engine's next calculation
                ...
                logAction(opponent.getName() + " rolled a " + aiRollSim + " and landed on " + cellTypeName.toUpperCase() + "!");
